...
function dniLetter( dni ) {
	if((dni.toString()).length === 8 && (typeof dni === 'number')) {
		var numero = dni % 23;
		var lockup = 'TRWAGMYFPDXBNJZSQVHLCKE';
		var result = lockup[numero];
		return result;
	} else {
...
function findLongestWord( array_words ){
	...
	for(var i=0; i<array_words.length; i++) {
		array_long.push(array_words[i].length);
	}
	for(var a=0; a<array_long.length; a++) {
		if(array_long[a] >= result) {
			result = array_long[a];
		}
	}
	return result;
...
array_lengths.push( findLongestWord( ["January", "February", "March", "April", "May", "June"] ) );
array_lengths.push( findLongestWord( ["One", "Two", "Three", "Four", "Five", "Six"] ) );
array_lengths.push( findLongestWord( ["Dog", "Car", "Elephant"] ) );
...
function sortArray( array ) {
    ...
    for (var a=0; a<array.length; a++) {
        if(array[a] === 'a' || array[a] === 'e' || array[a] === 'i' || array[a] === 'o' || array[a] === 'u') {
            array[a] = array[a].toLocaleUpperCase();
        }
    }
    return array;
}
